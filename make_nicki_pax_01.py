...
def gerb_stroke(g, stroke):
    dcode = 'D02' #move
    for (x,y) in stroke:
        g.append("G01X{0:d}Y{1:d}{2:s}*".format(round_gerb(x), round_gerb(y), dcode))
        dcode = 'D01' # draw to
    ...
    if len(stroke):
        ...
        for (x,y) in stroke:
            if move:
                ...
                g.append('G1F100' + xy2(x,y))

def cnc_write(strokes, file):
    lines = list()
    cnc_header(lines)
    for stroke in strokes:
        cnc_stroke(lines, stroke)
    cnc_footer(lines)
    write_file(lines, file)
...
cnc_write(master_strokes, name + ".nc")
